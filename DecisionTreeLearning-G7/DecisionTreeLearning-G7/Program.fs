...
let getHVBCData datasetFileName = 
    // 2: house-votes
...
let getSanitizedData datasetFileName = 
    ...
    else if (datasetFileName = "house-votes.txt" || datasetFileName = "breast-cancer.txt" || datasetFileName = "spect.txt") then dataset <- (getHVBCData datasetFileName)
    else
        printfn "No dataset found with the provided filename." 
    dataset
(*
let getDomainMap jsonFileName = 
    let mutable domMap = Map.empty
    let mutable path = ""
    ...
    else if (jsonFileName = "breast-cancer.txt") then path <- jsPath3
    else
        printfn "No json file found with the provided filename." 
        
    domMap <- domainMapping path 
*)
...
let main argv = 
    ...
    //let domMap = getDomainMap jsonFile
    printfn "%A" dataset
    printfn "%s" jsonFile
    printfn "%s" dataFile
    //printfn "%A" argv
